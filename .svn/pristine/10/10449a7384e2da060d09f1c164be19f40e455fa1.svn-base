...
public class Jaxb2Marshaller implements MimeMarshaller, MimeUnmarshaller,
		GenericMarshaller, GenericUnmarshaller, BeanClassLoaderAware,
		InitializingBean {
	...
	private Class<?>[] assignableClassesToScan;
	...
	public void setContextPaths(String... contextPaths) {
		...
		this.contextPath = StringUtils
				.arrayToDelimitedString(contextPaths, ":");
	}
	...
	public void setClassesToBeBound(Class<?>... classesToBeBound) {
		Assert.notEmpty(classesToBeBound,
				"'classesToBeBound' must not be empty");
	...
	public void setAssignableClassesToScan(Class<?>[] assignableClassesToScan) {
		this.assignableClassesToScan = assignableClassesToScan;
	}
	 ...
	 */
	public void setSchemaResourceResolver(
			LSResourceResolver schemaResourceResolver) {
	...
	public void afterPropertiesSet() throws Exception {
		...
		boolean hasClassesToBeBound = !ObjectUtils
				.isEmpty(this.classesToBeBound);
		...
		if (hasContextPath && (hasClassesToBeBound || hasPackagesToScan)
			...
			throw new IllegalArgumentException(
					"Specify either 'contextPath', 'classesToBeBound', "
							+ "or 'packagesToScan'");
		}
		if (!hasContextPath && !hasClassesToBeBound && !hasPackagesToScan) {
			throw new IllegalArgumentException(
	...
	private JAXBContext createJaxbContextFromContextPath() throws JAXBException {
		if (logger.isInfoEnabled()) {
			logger.info("Creating JAXBContext with context path ["
					+ this.contextPath + "]");
		}
		if (this.jaxbContextProperties != null) {
			if (this.beanClassLoader != null) {
				return JAXBContext.newInstance(this.contextPath,
						this.beanClassLoader, this.jaxbContextProperties);
			} else {
				// analogous to the JAXBContext.newInstance(String)
				// implementation
				return JAXBContext.newInstance(this.contextPath, Thread
	...
	private JAXBContext createJaxbContextFromClasses() throws JAXBException {
		if (logger.isInfoEnabled()) {
			logger.info("Creating JAXBContext with classes to be bound ["
					+ StringUtils
							.arrayToCommaDelimitedString(this.classesToBeBound)
					+ "]");
		}
		if (this.jaxbContextProperties != null) {
	...
	private JAXBContext createJaxbContextFromPackages() throws JAXBException {
		if (logger.isInfoEnabled()) {
			logger.info("Creating JAXBContext by scanning packages ["
					+ StringUtils
							.arrayToCommaDelimitedString(this.packagesToScan)
					+ "]");
		}
		Jaxb2TypeScanner scanner = new Jaxb2TypeScanner(this.beanClassLoader, assignableClassesToScan,
				this.packagesToScan);
		Class<?>[] jaxb2Classes = scanner.scanPackages();
		if (logger.isDebugEnabled()) {
			logger.debug("Found JAXB2 classes: ["
	...
	private Schema loadSchema(Resource[] resources, String schemaLanguage)
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Setting validation schema to "
					+ StringUtils
							.arrayToCommaDelimitedString(this.schemaResources));
		}
		...
		for (int i = 0; i < resources.length; i++) {
			...
			Assert.isTrue(resources[i].exists(), "Resource " + resources[i]
					+ " does not exist");
	...
	public boolean supports(Class<?> clazz) {
		return ((this.supportJaxbElementClass && JAXBElement.class
				.isAssignableFrom(clazz)) || supportsInternal(clazz,
				this.checkForXmlRootElement));
	}
			...
			if (JAXBElement.class.equals(parameterizedType.getRawType())
				...
				if (typeArgument instanceof Class) {
					...
					if (JdkVersion.getMajorJavaVersion() >= JdkVersion.JAVA_17
							&& classArgument.isArray()) {
						return classArgument.getComponentType().equals(
								Byte.TYPE);
					} else {
						return (isPrimitiveWrapper(classArgument)
								|| isStandardClass(classArgument) || supportsInternal(
									classArgument, false));
					}
	...
	private boolean supportsInternal(Class<?> clazz,
		...
		if (StringUtils.hasLength(this.contextPath)) {
			...
			String[] contextPaths = StringUtils.tokenizeToStringArray(
					this.contextPath, ":");
	...
	public void marshal(Object graph, Result result, MimeContainer mimeContainer)
		...
		try {
			...
			if (this.mtomEnabled && mimeContainer != null) {
				marshaller
						.setAttachmentMarshaller(new Jaxb2AttachmentMarshaller(
								mimeContainer));
			}
	...
	private void marshalStaxResult(Marshaller jaxbMarshaller, Object graph,
			Result staxResult) throws JAXBException {
		XMLStreamWriter streamWriter = StaxUtils.getXMLStreamWriter(staxResult);
		if (streamWriter != null) {
			jaxbMarshaller.marshal(graph, streamWriter);
		} else {
			XMLEventWriter eventWriter = StaxUtils
					.getXMLEventWriter(staxResult);
			if (eventWriter != null) {
	...
	protected void initJaxbMarshaller(Marshaller marshaller)
		...
		if (this.marshallerProperties != null) {
			for (String name : this.marshallerProperties.keySet()) {
				marshaller.setProperty(name,
						this.marshallerProperties.get(name));
			}
		}
	...
	public Object unmarshal(Source source, MimeContainer mimeContainer)
		...
		try {
			...
			if (this.mtomEnabled && mimeContainer != null) {
				unmarshaller
						.setAttachmentUnmarshaller(new Jaxb2AttachmentUnmarshaller(
								mimeContainer));
			}
	...
	protected Object unmarshalStaxSource(Unmarshaller jaxbUnmarshaller,
			Source staxSource) throws JAXBException {
		XMLStreamReader streamReader = StaxUtils.getXMLStreamReader(staxSource);
		if (streamReader != null) {
			return (this.mappedClass != null ? jaxbUnmarshaller.unmarshal(
					streamReader, this.mappedClass).getValue()
					: jaxbUnmarshaller.unmarshal(streamReader));
		} else {
			XMLEventReader eventReader = StaxUtils
					.getXMLEventReader(staxSource);
			if (eventReader != null) {
				return (this.mappedClass != null ? jaxbUnmarshaller.unmarshal(
						eventReader, this.mappedClass).getValue()
	...
	protected void initJaxbUnmarshaller(Unmarshaller unmarshaller)
		...
		if (this.unmarshallerProperties != null) {
			for (String name : this.unmarshallerProperties.keySet()) {
				unmarshaller.setProperty(name,
						this.unmarshallerProperties.get(name));
			}
		}
	...
	protected XmlMappingException convertJaxbException(JAXBException ex) {
		if (ex instanceof ValidationException) {
			return new ValidationFailureException("JAXB validation exception",
					ex);
		} else if (ex instanceof MarshalException) {
			return new MarshallingFailureException(
					"JAXB marshalling exception", ex);
		} else if (ex instanceof UnmarshalException) {
			return new UnmarshallingFailureException(
					"JAXB unmarshalling exception", ex);
		} else {
			// fallback
			return new UncategorizedMappingException("Unknown JAXB exception",
					ex);
		}
	}
	...
	private static class Jaxb2AttachmentMarshaller extends AttachmentMarshaller {
		...
		public String addMtomAttachment(byte[] data, int offset, int length,
			...
			ByteArrayDataSource dataSource = new ByteArrayDataSource(mimeType,
					data, offset, length);
			return addMtomAttachment(new DataHandler(dataSource),
					elementNamespace, elementLocalName);
		}

		@Override
		public String addMtomAttachment(DataHandler dataHandler,
	...
	private static class Jaxb2AttachmentUnmarshaller extends
		...
		public byte[] getAttachmentAsByteArray(String cid) {
			try {
				...
				return FileCopyUtils.copyToByteArray(dataHandler
						.getInputStream());
			} catch (IOException ex) {
				throw new UnmarshallingFailureException(
						"Couldn't read attachment", ex);
			}
		}
	...
	private static class ByteArrayDataSource implements DataSource {
		...
		public ByteArrayDataSource(String contentType, byte[] data, int offset,
				int length) {
